...
exports.setConfigDefaults = setConfigDefaults = (config, configPath) ->
  ...
  paths                = config.paths     ?= {}
  paths.root          ?= config.rootPath  ? process.cwd()
  ...
  conventions.ignored ?= paths.ignored ? (path) ->
    sysPath.basename(path).indexOf('_') is 0
...
exports.deepFreeze = deepFreeze = (object) ->
  Object.keys(Object.freeze object)
    .map (key) ->
      object[key]
    .filter (value) ->
      value? and typeof value is 'object' and not Object.isFrozen(value)
    .forEach(deepFreeze)
  object
...
exports.loadConfig = (configPath = 'config', options = {}) ->
  ...
  replaceSlashes config if sysPath.sep is '\\'
  normalizeConfig config
  deepFreeze config
  config
