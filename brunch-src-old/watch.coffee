...
# otherwise, return it.
# underscore.
exports.result = (object, property) ->
  value = object[property]
  if typeof value is 'function' then value.call(object) else value

# If object is an array, return it. Otherwise, return an array with object.
exports.ensureArray = (object) ->
...
getPluginIncludes = (plugins) ->
  plugins
    .map((plugin) -> exports.result plugin, 'include')
    .filter((paths) -> paths?)
    .reduce(((acc, elem) -> acc.concat(exports.ensureArray elem)), [])
...
initWatcher = (config, callback) ->
  ...
  async.filter watched, fs_utils.exists, (watchedFiles) ->
    watcher = chokidar.watch watchedFiles,
      ignored: ((path) -> /(^[.#]|(?:__|~)$)/.test sysPath.basename path),
...
#
# Returns nothing.
initialize = (configPath, configParams, onCompile, callback) ->
  helpers.loadPackages process.cwd(), (error, packages) ->
    return callback error if error?
    # Load config, get brunch packages from package.json.
    config     = helpers.loadConfig configPath, configParams
    ...
    initWatcher config, (error, watcher) ->
      ...
      callback error, {
        config, watcher, server, fileList, compilers, linters, compile
      }
...
#
# Returns nothing.
bindWatcherEvents = (config, fileList, compilers, linters, watcher, onChange) ->
  watcher
    .on 'add', (path) ->
      # Update file list.
      onChange()
...
class BrunchWatcher
  constructor: (persistent, options, onCompile) ->
    configParams = {optimize: false, minify: false, persistent: false}
    initialize options, configParams, onCompile, (error, result) =>
      return logger.error error if error?
      {config, watcher, fileList, compilers, linters, compile} = result
      logger.notifications = config.notifications
      bindWatcherEvents config, fileList, compilers, linters, watcher, @_startCompilation
